...
#define _TINYLOG_IF_HAS_ANSI_COLORS(x) x
#else
#define _TINYLOG_IF_HAS_ANSI_COLORS(x)
...
inline constexpr char k_reset_ansi_colors[1] = "";
#endif
...
inline std::ofstream g_log_file = [] () -> std::ofstream {
    if (auto path = std::getenv("TINYLOG_FILE")) {
...
inline void output_log(log_level lev, std::string msg, std::source_location const &loc) {
    ...
    if (lev >= g_max_level) {
        std::cout << _TINYLOG_IF_HAS_ANSI_COLORS(k_level_ansi_colors[(std::uint8_t)lev] +)
                                                 msg _TINYLOG_IF_HAS_ANSI_COLORS(+ k_reset_ansi_color) + '\n';
    }
}
...
void generic_log(log_level lev, details::with_source_location<std::format_string<Args...>> fmt, Args &&...args) {
    if (lev >= details::g_max_level) {
        ...
        // std::cout TINYLOG_IF_HAS_ANSI_COLORS(<< "\E[" << details::level_ansi_colors[(std::uint8_t)lev])
        //     << loc.file_name() << ":" << loc.line() << " [" << details::log_level_name(lev)<< "] "
        //     << std::vformat(fmt.format().get(), std::make_format_args(args...))
        //     TINYLOG_IF_HAS_ANSI_COLORS(<< "\E[m") << '\n';
